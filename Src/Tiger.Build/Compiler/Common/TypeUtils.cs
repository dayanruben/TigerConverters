...
namespace Tiger.Build.Compiler.Common
{
    internal static class TypeUtils
    {
        ...
        internal static MethodInfo GetUserDefinedCoercionMethod(Type convertFrom, Type convertToType, bool implicitOnly)
        {
            ...
            Type nnExprType = convertFrom.GetNonNullableType();
            Type nnConvType = convertToType.GetNonNullableType();
            // try exact match on types
            MethodInfo[] eMethods =
                nnExprType.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
            ...
            MethodInfo[] cMethods =
                nnConvType.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
        ...
        internal static MethodInfo GetBooleanOperator(Type type, string name)
        {
            do
            {
                MethodInfo result = type.GetMethodValidated(name, AnyStatic, null, new[] {type}, null);
        ...
        {
            // we don't have to scan base or declaring types here.
        ...
        public static Type GetCollectionType(Type type)
        {
            ...
            MethodInfo enumerator = type.GetMethod("GetEnumerator");

            if (enumerator == null) throw new ArgumentException();
            Type enumeratorResult = enumerator.ReturnType;

            MethodInfo moveNext = enumeratorResult.GetMethod("MoveNext");
            PropertyInfo current = enumeratorResult.GetProperty("Current");
